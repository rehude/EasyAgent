...
详细的循环结构、文件分工、关键不变量见 [.Codex/docs/architecture.md](.Codex/docs/architecture.md)。
...
- **readline completer 必须同步**(arity=1,直接 `return [hits, line]`)。Node 24 的 `readline/promises` 异步回调路径有 bug,会在 `_tabCompleter` 解构 undefined 时崩。详见 [.Codex/docs/architecture.md](.Codex/docs/architecture.md) 的"补全器"节。
...
- [.Codex/docs/architecture.md](.Codex/docs/architecture.md) — REPL/Agent 循环、注册表、会话、流式渲染、补全器细节
- [.Codex/docs/conventions.md](.Codex/docs/conventions.md) — TS / ESM / 提交信息 / 工具与命令的添加方式
